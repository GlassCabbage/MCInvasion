...
namespace MCInvasion.NPCs
{
	public class Ghost : ModNPC
	{
        ...
        {
			npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<powder>(), 1,50,100));
			npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<GhostHead>(), 2));
			npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<GhostPetItem>(), 4));
		}
		...
		private void close(int dir)
		{
			...
			NPC.TargetClosest(true);
			if (NPC.Center.Y - Main.player[NPC.target].Center.Y > -250+fixy)
				NPC.velocity.Y = -5;
			else if (NPC.Center.Y - Main.player[NPC.target].Center.Y < -300+fixy)
				NPC.velocity.Y = 5;
			else
				NPC.velocity.Y = 0;
			...
			if (AI_Time2<=0 && Math.Abs(NPC.rotation)<0.01 && (Math.Abs(NPC.Center.X - Main.player[NPC.target].Center.X) <= closeDistance + fixx) && Math.Abs(NPC.Center.X - Main.player[NPC.target].Center.X) >= leaveDistance && NPC.Center.Y - Main.player[NPC.target].Center.Y <= -250 + fixy && NPC.Center.Y - Main.player[NPC.target].Center.Y >= -300 + fixy)
			{
				NPC.rotation=0;
				AI_Timer = 0;
				NPC.velocity.X = 0;
		...
		private void shoot()
		{
			...
			if (AI_Timer < hold) { 
				if (AI_Timer % 5 == 0)
					for (int i = -15; i < 15; i++)
					{
						Vector2 face = new Vector2(52 * NPC.direction, -32);
			...
			if (AI_Timer > hold+120)
			{
				...
				if (AI_Timer == hold+153)
				{
					...
					Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, 10F * Main.player[NPC.target].DirectionFrom(NPC.Center), ModContent.ProjectileType<GhostFireBall>(), (int)100, 0f, Main.myPlayer);
				}
